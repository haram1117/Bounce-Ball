...
public class BallBounce : MonoBehaviour
{
    ...
    {
        var snapshot = await FirebaseDatabase.DefaultInstance.GetReference("Users").Child(id).Child("death").GetValueAsync().ConfigureAwait(false);
    ...
    void NextLevel()
    {
        if (nextLevel == true)
        {
            reference.Child("Users").Child(id).Child("stage").SetValueAsync(nowStage-2);
    ...
    public void Respawn()
    {
        ...
        {
            death++;
            reference.Child("Users").Child(id).Child("death").SetValueAsync(death);
            star = GameObject.Find("StarField").transform.childCount;
